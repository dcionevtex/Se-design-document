...
The final `.docx` must:
- Follow VTEX brand guidelines — see **VTEX Brand Skill** below
...
## VTEX Brand Skill — Required Companion

This skill is designed to work in combination with the **VTEX Brand Skill**:

> 🔗 **https://github.com/dcionevtex/vtex-brand-skill**

Install and load this companion skill to ensure all generated documents apply the correct VTEX visual identity — colors, typography, cover page layout, table styles, and brand voice. Without it, documents will be structurally correct but visually generic.

**What the VTEX Brand Skill provides:**
- Official color palette: Rebel Pink `#F71963`, Serious Black `#142032`, Soft Blue `#F5F9FF`
- Typography system: VTEX Trust font family, heading hierarchy, weight rules
- Document layout: cover page template, header/footer patterns, section divider styles
- Table styling: header colors, alternating rows, border conventions
- Brand voice: bold, trustworthy, optimistic — customer as protagonist, VTEX as guide
- python-pptx and docx generation patterns pre-configured for VTEX brand

**Integration instructions:**
1. Install the VTEX Brand Skill from the repo above
2. When generating any SDD `.docx`, load both skills — this skill drives content and structure, the brand skill drives visual output
3. Reference the brand skill's `references/formatting-guide.md` for all style decisions
4. Use the brand skill's cover page template for the SDD cover
